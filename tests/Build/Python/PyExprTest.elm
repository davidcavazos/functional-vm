...
import Dict
import Expect
import FVM.Build.Python exposing (pyExpression)
import Test exposing (Test, describe, test)


suite : Test
suite =
    describe "build Python -- pyExpression"
        -- Type
        [ describe "Type"
            [ test "type -- Type" <|
                \_ ->
                    pyExpression (Type TypeT)
                        |> Expect.equal "Type"
            ]
        ...
        , describe "Tuple"
            ...
            , test "tuple with one item -- (1)" <|
                \_ ->
                    pyExpression (Tuple [ Int 1 ])
                        |> Expect.equal "(1,)"

            --
            , test "tuple with many items -- (1, 2, 3)" <|
                \_ ->
                    pyExpression (Tuple [ Int 1, Int 2, Int 3 ])
                        |> Expect.equal "(1,2,3)"
            ]
        ...
        , describe "Record"
            [ test "record empty -- {}" <|
                \_ ->
                    pyExpression (Record Dict.empty)
                        |> Expect.equal "Record()"

            --
            , test "record with one item -- {x = 1}" <|
                \_ ->
                    pyExpression (Record (Dict.singleton "x" (Int 1)))
                        |> Expect.equal "Record(x=1)"

            --
            , test "record with many items -- {x = 1, y = 2, z = 3}" <|
                \_ ->
                    pyExpression (Record (Dict.fromList [ ( "x", Int 1 ), ( "y", Int 2 ), ( "z", Int 3 ) ]))
        ...
        , describe "Let"
            [ test "let empty -- 1" <|
                \_ ->
                    pyExpression (Let Dict.empty (Int 1))
                        |> Expect.equal "1"

            --
            , test "let one variable -- let x = 1; 2" <|
                \_ ->
                    pyExpression (Let (Dict.singleton "x" (Int 1)) (Int 2))
                        |> Expect.equal "(lambda x:2)(1)"

            --
            , test "let many variables -- let x = 1; let y = 2; let y = 3; 4" <|
                \_ ->
                    pyExpression (Let (Dict.fromList [ ( "x", Int 1 ), ( "y", Int 2 ), ( "z", Int 3 ) ]) (Int 4))
        ...
        , describe "Function"
            [ test "function without inputs -- () -> 1" <|
                \_ ->
                    pyExpression (Function Dict.empty (Int 1))
                        |> Expect.equal "lambda:1"

            --
            , test "function with one input -- x -> 1" <|
                \_ ->
                    pyExpression (Function (Dict.singleton "x" IntT) (Int 1))
                        |> Expect.equal "lambda x:1"

            --
            , test "function with many inputs -- x y z -> 1" <|
                \_ ->
                    pyExpression (Function (Dict.fromList [ ( "x", IntT ), ( "y", IntT ), ( "z", IntT ) ]) (Int 1))
        ...
        , describe "Call"
            [ test "call without inputs -- let f : () -> Int; f" <|
                \_ ->
                    pyExpression (Call (Load "f" (FunctionT [] IntT)) [])
                        |> Expect.equal "f()"

            --
            , test "call with one input -- let f : Int -> Int; f 1" <|
                \_ ->
                    pyExpression (Call (Load "f" (FunctionT [ IntT ] IntT)) [ Int 1 ])
                        |> Expect.equal "f(1)"

            --
            , test "call with many inputs -- let f : Int -> Int -> Int -> Int; f 1 2 3" <|
                \_ ->
                    pyExpression (Call (Load "f" (FunctionT [ IntT, IntT, IntT ] IntT)) [ Int 1, Int 2, Int 3 ])
                        |> Expect.equal "f(1,2,3)"

            --
            , test "call with currying -- let f : Int -> Int -> Int -> Int; f 1" <|
                \_ ->
                    pyExpression (Call (Load "f" (FunctionT [ IntT, IntT, IntT ] IntT)) [ Int 1 ])
        ...
        , describe "Constructor"
            [ test "constructor without inputs -- T.A" <|
                \_ ->
                    pyExpression (Constructor ( "T", [] ) "A" [])
                        |> Expect.equal "A()"

            --
            , test "constructor with one input -- T.A 1" <|
                \_ ->
                    pyExpression (Constructor ( "T", [] ) "A" [ Int 1 ])
                        |> Expect.equal "A(1)"

            --
            , test "constructor with many inputs -- T.A 1 2 3" <|
                \_ ->
                    pyExpression (Constructor ( "T", [] ) "A" [ Int 1, Int 2, Int 3 ])
